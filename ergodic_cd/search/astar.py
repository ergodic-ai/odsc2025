...
class AStarSearch:
    ...
    def __init__(
        ...
        super_graph = self._augment_super_graph_with_tiers(super_graph, tiers)
    ...
    def _augment_super_graph_with_tiers(self, super_graph, tiers):
        n_tiers = len(tiers)

        if n_tiers < 2:
            return super_graph

        print("augmenting super graph with tiers")

        for i in range(n_tiers - 1):
            cause_tier = tiers[i]
            for j in range(i + 1, n_tiers):
                effect_tier = tiers[j]
                for cause_node in cause_tier:
                    for effect_node in effect_tier:
                        super_graph.loc[effect_node, cause_node] = 0
